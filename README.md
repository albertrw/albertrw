<!-- Header -->
<p align="center">
  <img src="https://raw.githubusercontent.com/albertrw/albertrw/main/github-banner.jpg" alt="Albert Abayisenga - Software Developer" width="100%" />
</p>

# 👋 Hi, I'm Albert Abayisenga (albertrw)

💻 Software Developer | Graphic Designer | Photographer | Videographer  
🏢 Co-Director at [Bakame Labs](https://bakamelabs.com)

---

## 🍽️ Featured Project — Digital Menu System

### 🧠 Overview
**Digital Menu** is a modern, responsive web application designed for restaurants and lodges.  
It allows guests to browse menu items, view details, and place orders digitally — enhancing user experience and service speed.

### ⚙️ Tech Stack
- **Frontend:** EJS, Tailwind CSS, JavaScript  
- **Backend:** Node.js, Express.js  
- **Database:** MySQL  
- **Other:** AJAX, Dynamic Data Tables, SweetAlert, PDF Export

### 🌟 Key Features
✅ View menu items with images and details  
✅ Filter and sort menu items by name or price  
✅ Admin dashboard for adding/editing/deleting items  
✅ PDF/CSV export of menu and services  
✅ AJAX for real-time updates without page reload  
✅ Mobile-ready UI built with Tailwind CSS  

> 💡 The system was designed to support fast, secure, and real-time operations with scalability in mind — making it easy to integrate into mobile and desktop apps later.

🔗 **Repository:** [Digital Menu System](https://github.com/albertrw/digital-menu-system)  
🌐 **Demo:** [bakamelabs.com/menu](https://bakamelabs.com/project/5)

---

## 🧰 Skills & Tools

| Category | Technologies |
|-----------|---------------|
| **Languages** | JavaScript, PHP, Python, C, Dart |
| **Frameworks** | Node.js, Express, Flutter, Bootstrap, Tailwind |
| **Database** | MySQL, SQLite |
| **Tools** | Git, VS Code, Figma, Adobe Photoshop, Premiere Pro |

---

## 🏗️ Current Projects
- 🎮 **Coin Miner Game** (Node.js + MySQL)
- 🏥 **Clinic Management System** (AJAX + Node.js + MySQL)
- 👕 **Bakame Wear** — Clothing Brand  
- 💧 **Chez Drocella Eglantine Lodge** Management System

---

## 📈 GitHub Stats

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=albertrw&show_icons=true&theme=tokyonight" height="160">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=albertrw&theme=tokyonight" height="160">
</p>

---

## 📫 Contact
🌍 **Portfolio:** [albertrw.github.io/albert-portfolio](https://albertrw.github.io/albert-portfolio/)  
✉️ **Email:** abayisengaalbert@gmail.com  
📱 **Phone/WhatsApp:** +250 792 044 484  
🔗 [LinkedIn](https://linkedin.com/in/abayisengaalbert) | [Instagram](https://www.instagram.com/derekpro_/)

---

> 🧩 *“Building technology that makes daily experiences smarter and smoother.”*
